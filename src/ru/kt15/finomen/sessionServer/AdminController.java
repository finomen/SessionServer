...
public class AdminController implements AdminHandler, Runnable {
	...
	private PacketListener udpHandler = new PacketListener() {
		...
		public void handlePacket(PacketConnection conn, InetSocketAddress source,
			...
			if (logUdp) {
				appendUdpPacketLog(Packet.newBuilder().setData(ByteString.copyFrom(packet))
						.setDestHost(dest.getAddress().getHostAddress()).setDestPort(dest.getPort())
						.setSourceHost(source.getAddress().getHostAddress()).setSourcePort(source.getPort()).build());
			}
		}
	};
	...
	private PacketListener tcpHandler = new PacketListener() {
		...
		public void handlePacket(PacketConnection conn, InetSocketAddress source,
			...
			if (logTcp) {
				appendTcpPacketLog(Packet.newBuilder().setData(ByteString.copyFrom(packet))
						.setDestHost(dest.getAddress().getHostAddress()).setDestPort(dest.getPort())
						.setSourceHost(source.getAddress().getHostAddress()).setSourcePort(source.getPort()).build());
			}
		}
	...
	public void run() {
		while (!Thread.interrupted()) {
			...
			synchronized (packetLock) {
				...
				ByteBuffer buf = ByteBuffer.allocate(sc.getSerializedSize() + 3);
