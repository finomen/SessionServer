...
public class Main {
	...
	public static void main(String[] args) throws IOException, InterruptedException {
		...
		for(NetworkInterface ifc : Collections.list(NetworkInterface.getNetworkInterfaces())) {
		   if(ifc.isUp()) {
		      for(InetAddress addr : Collections.list(ifc.getInetAddresses())) {
		    	  ...
		    	  DatagramConnection conn = new DatagramConnection(ioService, true, new InetSocketAddress(addr, Options.udpPort));
		...
		ClientPacketListener clientListener = new ClientPacketListener(replicationListener, adminController);
		System.out.println("OK");
		System.out.println("Start listening...");
		ServerConnection serverConnection = new ServerConnection(new ClientProtocolDefinition(), ioService, Options.tcpPort);
